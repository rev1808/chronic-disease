...
class DiseasePrediction:
    ...
    def _load_test_dataset(self):
        print("Loading Test Dataset from path:", self.config['dataset']['test_data_path'])  # Add this line
    ...
    def _feature_correlation(self, data_frame=None, show_fig=False):
        ...
        numeric_data = data_frame.drop(columns=['prognosis'])#.select_dtypes(include=[np.number])
    ...
    def plot_validation_curve(self):
        ...
        train_scores, test_scores = validation_curve(
            self.clf, self.train_features, self.train_labels, 
    ...
    def plot_learning_curve(self):
        train_sizes, train_scores, test_scores = learning_curve(
            self.clf, self.train_features, self.train_labels, cv=5, 
    ...
    def plot_confusion_matrix(self, conf_mat):
        ...
        sn.heatmap(conf_mat, annot=True, fmt='g', cmap='Blues', xticklabels=self.clf.classes_,
                   yticklabels=self.clf.classes_)
        plt.title(f'Confusion Matrix, Model name: {self.model_name }')
...
if __name__ == "__main__":
    ...
    dp.plot_validation_curve()  # Plot Validation Curve
    dp.plot_learning_curve()  # Plot Learning Curve
